...
if(isset($_GET['brgyPic'])){
    if(isset($_FILES['pp']) && $_FILES['pp']['tmp_name'] != ''){//checks if their are files submitted from the form
        ...
        $data = "barangay_pic = '$fnamep' "; //saves filename to $data
        $barangay = $_SESSION['userBarangay']; //$_SESSION value for UsersID (prmry key) held to $usersID
    }

    if(isset($data)){//checks if the previous if statement was successful
        $sql = "UPDATE barangay set $data where BarangayName = '$barangay'";           //sql statement 
        ...
        if(!mysqli_stmt_prepare($stmt, $sql)){                              //if it fails, send back to profile.php to error msg
            // header("location: ../barangay.php?error=stmtfailedupdateprofile");
            // exit();
        ...
        header("location: ../barangay.php?error=none"); //no errors were made
        exit();
    }
}
