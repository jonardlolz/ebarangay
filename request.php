                ...
                <div class="table-responsive">
                    <table class="table table-bordered text-center text-dark" 
                        ...
                        <tbody>
                            <!--Row 1-->
                            <?php 
                                $requests = $conn->query("SELECT request.*, concat(users.Firstname, 
                                ' ', users.Lastname) as name, DATE_FORMAT(requestedOn, 
                                '%m/%d/%Y %h:%i %p') as requestedDate, DATE_FORMAT(approvedOn, 
                                '%m/%d/%Y %h:%i %p') as approvedDate, users.userType, 
                                users.profile_pic FROM request INNER JOIN users ON 
                                request.UsersID=users.UsersID WHERE request.status='Approved' 
                                AND request.userBarangay='{$_SESSION['userBarangay']}' AND 
                                request.userType='{$_SESSION['userType']}'");
                                while($row=$requests->fetch_assoc()):
                                    if($row["userType"] == "Admin"){
                                        continue;
                                    }
                            ...
                            <tr>
                                ...
                                <td><?php echo $row["requestedDate"] ?></td>
                                <td><?php if($row["status"] != NULL){echo $row["status"];} else{echo "Pending";} ?></td>
                                <td><a href="includes/request.inc.php?approveID=<?php echo $row["RequestID"] ?>"><i class="fas fa-check fa-2x"></i></a>/ <a href="includes/request.inc.php?declineID=<?php echo $row["RequestID"] ?>"><i class="fas fa-times fa-2x"></i></a></td>
                                <!--Right Options-->
                            </tr>
                            <?php endwhile; ?>
                            <!--Row 1-->
                ...
                <div class="table-responsive">
                    <table class="table table-bordered text-center text-dark" 
                        ...
                        <tbody>
                            <!--Row 1-->
                            <?php 
                                $requests = $conn->query("SELECT request.*, concat(users.Firstname, ' ', users.Lastname) as name, DATE_FORMAT(requestedOn, '%m/%d/%Y %h:%i %p') as requestedDate, DATE_FORMAT(approvedOn, '%m/%d/%Y %h:%i %p') as approvedDate, users.userType, users.profile_pic FROM request INNER JOIN users ON request.UsersID=users.UsersID WHERE request.status='Approved';");
                                while($row=$requests->fetch_assoc()):
                                    if($row["userType"] == "Admin"){
                                        continue;
                                    }
                ...
                <div class="table-responsive">
                    <table class="table table-bordered text-center text-dark" 
                        ...
                        <tbody>
                            ...
                            <?php 
                                $requests = $conn->query("SELECT request.*, concat(users.Firstname, ' ', 
                                users.Lastname) as name, DATE_FORMAT(requestedOn, '%m/%d/%Y %h:%i %p') as 
                                requestedDate, DATE_FORMAT(approvedOn, '%m/%d/%Y %h:%i %p') as approvedDate, 
                                users.userType, users.profile_pic FROM request INNER JOIN users ON 
                                request.UsersID=users.UsersID WHERE request.status='Pending' 
                                AND request.userBarangay='{$_SESSION['userBarangay']}' 
